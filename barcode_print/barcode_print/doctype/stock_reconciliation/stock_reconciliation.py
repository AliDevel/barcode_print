...
@frappe.whitelist()
def get_items(
	warehouse, posting_date, posting_time, company, item_group=None, ignore_empty_stock=False
):
    ...
    # Additional filters or conditions can be applied here
...
def get_itemss(
	...
	ignore_empty_stock = cint(ignore_empty_stock)
	items = [frappe._dict({"item_group": item_group, "warehouse": warehouse})]

	if not item_group:
		items = get_items_for_stock_reco(warehouse, company)

	res = []
	itemwise_batch_data = get_itemwise_batch(warehouse, posting_date, company, item_group)

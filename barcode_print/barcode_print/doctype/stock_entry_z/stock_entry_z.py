...
class StockEntryZ(StockController):
	def __init__(self, *args, **kwargs):
		super(StockEntryZ, self).__init__(*args, **kwargs)
	...
	def on_submit(self):
		...
		#date_serial_nos_after_submit(self, "items")
	...
	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in frappe.get_all(
				...
				frappe.delete_doc("Stock Entry Z", d.name)
	...
	def update_cost_in_project(self):
		...
		if self.project:
			amount = frappe.db.sql(
				...
				from
					`tabStock Entry Z` se, `tabStock Entry Detail` sed
				where
			...
			additional_costs = frappe.db.sql(
				...
				from
					`tabStock Entry Z` se, `tabLanded Cost Taxes and Charges` sed
				where
					se.docstatus = 1 and se.project = %s and sed.parent = se.name
					...
					if cstr(item.t_warehouse) == "":
						...
						transferred_materials = frappe.db.sql(
							"""
									select
										sum(qty) as qty
									from `tabStock Entry Z` se,`tabStock Entry Detail` sed
									where
	...
	def validate_difference_account(self):
		...
		for d in self.get("items"):
			if not d.expense_account:
				...
				frappe.throw(
					_(
						"Difference Account must be a Asset/Liability type account, since this Stock Entry Z is an Opening Entry"
					),
					OpeningEntryAccountError,
				)
	...
	def validate_subcontract_order(self):
		...
		if backflush_raw_materials_based_on == "BOM":
			...
			for se_item in self.items:
				...
				se = frappe.qb.DocType("Stock Entry Z")
	...
	def get_gl_entries(self, warehouse_account):
		gl_entries = super(StockEntryZ, self).get_gl_entries(warehouse_account)
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == "Material Transfer":
			doc = frappe.get_doc("Stock Entry Z", self.outgoing_stock_entry)
	...
	def get_items(self):
		...
		if self.bom_no:
			...
			# fetch the serial_no of the first Stock Entry Z for the second Stock Entry Z
			if self.work_order and self.purpose == "Manufacture":
	...
	def get_used_scrap_items(self):
		...
		data = frappe.get_all(
			...
			filters=[
				["Stock Entry Z", "work_order", "=", self.work_order],
				["Stock Entry Detail", "is_scrap_item", "=", 1],
				["Stock Entry Z", "docstatus", "=", 1],
				["Stock Entry Z", "purpose", "in", ["Repack", "Manufacture"]],
			],
		)
	...
	def get_pro_order_required_items(self, backflush_based_on=None):
		"""
		Gets Work Order Required Items only if Stock Entry Z purpose is **Material Transferred for Manufacture**.
		"""
	...
	def update_transferred_qty(self):
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			...
			args = {
				...
				"target_parent_dt": "Stock Entry Z",
	...
	def set_material_request_transfer_status(self, status):
		...
		if self.outgoing_stock_entry:
			parent_se = frappe.get_value("Stock Entry Z", self.outgoing_stock_entry, "add_to_transit")
	...
	def get_serial_nos_for_fg(self, args):
		...
		filters = [
			["Stock Entry Z", "work_order", "=", self.work_order],
			["Stock Entry Z", "purpose", "=", "Manufacture"],
			["Stock Entry Z", "docstatus", "=", 1],
			["Stock Entry Detail", "item_code", "=", self.pro_doc.production_item],
		]

		stock_entries = frappe.get_all("Stock Entry Z", fields=fields, filters=filters)
...
def move_sample_to_retention_warehouse(company, items):
	...
	stock_entry = frappe.new_doc("Stock Entry Z")
...
def make_stock_in_entry(source_name, target_doc=None):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = frappe.db.get_value("Stock Entry Z", source_name, "add_to_transit")
			if add_to_transit:
...
def get_used_alternative_items(
	...
	data = frappe.db.sql(
		...
		from
			`tabStock Entry Z` ste, `tabStock Entry Detail` sted
		where
...
def get_supplied_items(
	...
	filters = [
		["Stock Entry Z", "docstatus", "=", 1],
		["Stock Entry Z", subcontract_order_field, "=", subcontract_order],
	]

	supplied_item_details = {}
	for row in frappe.get_all("Stock Entry Z", fields=fields, filters=filters):
...
def get_stock_entry_data(work_order):
	stock_entry = frappe.qb.DocType("Stock Entry Z")
